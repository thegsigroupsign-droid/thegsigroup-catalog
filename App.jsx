...
export default function App() {
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      if (editingId) {
        await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'products', editingId), { ...newProduct, price: Number(newProduct.price), updatedAt: new Date().toISOString() });
        setEditingId(null);
      } else {
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'products'), { ...newProduct, price: Number(newProduct.price), createdAt: new Date().toISOString() });
      }
  ...
  const filteredProducts = useMemo(() => {
    ...
    return res.sort((a, b) => sortBy === "price-low" ? a.price - b.price : sortBy === "price-high" ? b.price - a.price : new Date(b.createdAt) - new Date(a.createdAt));
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-orange-100 overflow-x-hidden">
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 h-20 flex justify-between items-center px-4 sm:px-8 shadow-sm">
        ...
        <div className="hidden md:flex flex-1 max-w-md mx-8 relative">
          ...
          <input 
            ...
            className="w-full bg-slate-100/50 border border-slate-200 py-2.5 pl-11 pr-4 rounded-2xl text-sm outline-none focus:bg-white focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all"
            value={searchTerm}
          ...
          <div className="mb-12 space-y-8 animate-in slide-in-from-top-6 duration-700 text-left">
            ...
            <div className="bg-white rounded-[2rem] p-6 sm:p-8 shadow-sm border border-slate-100">
              ...
              <form onSubmit={handleAddVideo} className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <input placeholder="YouTube URL (e.g. https://youtu.be/...)" className="p-4 bg-slate-50 rounded-2xl text-sm outline-none border border-slate-100" value={newVideo.youtubeUrl} onChange={e => setNewVideo({...newVideo, youtubeUrl: e.target.value})} required />
            ...
            <div className="bg-slate-900 rounded-[2.5rem] p-6 sm:p-8 text-white shadow-2xl">
              <h2 className="text-xl font-bold mb-6 flex items-center gap-3 text-orange-500 uppercase italic tracking-tighter">
              ...
              <form onSubmit={handleAddBanner} className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <input placeholder="Slide Main Title" className="p-4 bg-white/5 border border-white/10 rounded-2xl text-sm outline-none focus:border-orange-500" value={newBanner.title} onChange={e => setNewBanner({...newBanner, title: e.target.value})} required />
                <input placeholder="Background Image URL" className="p-4 bg-white/5 border border-white/10 rounded-2xl text-sm outline-none focus:border-orange-500" value={newBanner.image} onChange={e => setNewBanner({...newBanner, image: e.target.value})} required />
                <textarea placeholder="Slide Subtitle" className="md:col-span-2 p-4 bg-white/5 border border-white/10 rounded-2xl text-sm outline-none h-20" value={newBanner.subtitle} onChange={e => setNewBanner({...newBanner, subtitle: e.target.value})} required />
                <button type="submit" className="md:col-span-2 bg-orange-600 p-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-orange-500">Create New Banner Slide</button>
              </form>
                  ...
                  <div key={b.id} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/10 group">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-white/40">{b.subtitle}</p>
                      </div>
                    </div>
            ...
            <div className="bg-white rounded-[2rem] p-6 sm:p-8 shadow-sm border border-slate-100">
               <h2 className="text-xl font-bold mb-6 flex items-center gap-3 text-slate-900 uppercase italic tracking-tighter">
                <Settings className="w-5 h-5" /> Global Site Settings
              </h2>
              <form onSubmit={handleSaveSettings} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input className="p-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm outline-none" placeholder="Company Name" value={editSettings.companyName} onChange={e => setEditSettings({...editSettings, companyName: e.target.value})} />
                <input className="p-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm outline-none" placeholder="Tagline" value={editSettings.tagline} onChange={e => setEditSettings({...editSettings, tagline: e.target.value})} />
                <button type="submit" className="md:col-span-2 bg-slate-900 text-white p-4 rounded-2xl font-black uppercase text-xs tracking-widest">Update Branding</button>
              </form>
            </div>

            {/* Adicionar Produto */}
            <div className="bg-white border-2 border-orange-100 rounded-[2.5rem] p-6 sm:p-8 shadow-2xl text-left">
              <h2 className="text-xl font-black italic uppercase tracking-tighter mb-8 flex items-center gap-2 text-orange-600">
                <PlusCircle className="w-5 h-5" /> Publish New Work
              </h2>
              <form onSubmit={handleAddProduct} className="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <select className="p-4 bg-slate-50 rounded-2xl text-sm outline-none border border-slate-100" value={newProduct.category} onChange={e => setNewProduct({...newProduct, category: e.target.value})}>
                ...
                <button type="submit" className="md:col-span-3 bg-slate-900 text-white py-4 rounded-[1.5rem] font-black uppercase text-xs tracking-widest">Add to Catalog</button>
              </form>
            </div>
          </div>
        )}
        ...
        {!isAdminMode && (
          <div className="mb-10 relative h-[350px] sm:h-[550px] rounded-[2rem] sm:rounded-[3.5rem] overflow-hidden group shadow-2xl">
            {banners.map((slide, index) => (
              ...
              >
                {/* Overlay para contraste se houver imagem */}
                ...
                <div className="relative z-10 max-w-2xl text-left text-white">
                  <div className="inline-flex items-center gap-2 bg-white/20 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-6 backdrop-blur-md border border-white/10 animate-in slide-in-from-left-4 duration-700 delay-200">
                    <Award className="w-3 h-3"/> {siteSettings.badgeText}
                  </div>
                  <h2 className="text-3xl sm:text-7xl font-black mb-6 uppercase tracking-tighter leading-[0.9] animate-in slide-in-from-bottom-4 duration-700 delay-300">
                  ...
                  <p className="text-white/90 mb-10 text-sm sm:text-xl font-medium opacity-90 leading-relaxed max-w-md animate-in slide-in-from-bottom-4 duration-700 delay-400">
                    {slide.subtitle}
                  </p>
                  <button onClick={() => setIsAiChatOpen(true)} className="bg-white text-slate-900 px-10 py-4 rounded-full font-black shadow-2xl uppercase text-xs tracking-widest hover:scale-105 transition-all animate-in slide-in-from-bottom-4 duration-700 delay-500">
                    Start Project
                  </button>
                </div>
              </div>
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex gap-2">
              {banners.map((_, i) => (
                <button 
                  ...
                  className={`h-1.5 rounded-full transition-all duration-500 ${i === currentSlide ? 'w-8 bg-white' : 'w-2 bg-white/30 hover:bg-white/50'}`}
                />
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {videos.map(v => (
                <div key={v.id} className="bg-white rounded-[2rem] p-4 shadow-sm border border-slate-100 hover:shadow-xl transition-all">
                  ...
                  <h3 className="font-bold text-slate-800 uppercase tracking-tighter px-2">{v.title}</h3>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-col gap-6 mb-12">
          ...
          <div className="flex items-center justify-between px-4 border-t border-slate-200 pt-6">
            <div className="flex items-center gap-2 text-slate-400 font-black text-[10px] uppercase tracking-widest">
              {filteredProducts.length} Portfolio Items
            </div>
          ...
          {filteredProducts.map(product => (
            <div key={product.id} className="group bg-white rounded-[2.5rem] border border-slate-100 p-4 hover:shadow-2xl transition-all duration-500 flex flex-col shadow-sm">
              <div className="aspect-square rounded-[2rem] overflow-hidden bg-slate-50 mb-6 relative">
                ...
                <div className="absolute top-4 left-4 bg-orange-600 text-white text-[8px] font-black px-3 py-1.5 rounded-lg uppercase tracking-wider">
              ...
              <div className="flex-1 flex flex-col px-2">
                ...
                <div className="flex justify-between items-center mt-auto pt-5 border-t border-slate-50">
                  <div className="flex flex-col"><span className="text-[9px] text-slate-400 font-bold uppercase leading-none">Price</span><span className="text-xl font-black text-slate-900">${Number(product.price).toLocaleString()}</span></div>
      ...
      <footer className="bg-slate-900 text-white py-16 mt-20 text-center px-4 relative">
        <h2 className="text-3xl font-black italic uppercase tracking-tighter mb-4">{siteSettings.companyName.split(' ')[0]} <span className="text-orange-500">{siteSettings.companyName.split(' ').slice(1).join(' ')}</span></h2>
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-8 mb-12 text-slate-500 text-[10px] font-black uppercase tracking-widest text-left">
          <div className="flex items-center gap-2"><MapPin size={16} className="text-orange-500" /> {siteSettings.address}</div>
          <div className="flex items-center gap-2"><Mail size={16} className="text-orange-500" /> {siteSettings.email}</div>
          <div className="flex items-center gap-2"><Phone size={16} className="text-orange-500" /> +1 {siteSettings.whatsapp}</div>
        </div>
        ...
        <div className="bg-white h-full sm:h-[650px] sm:rounded-[3rem] shadow-2xl w-full sm:w-[400px] flex flex-col border border-slate-100 overflow-hidden text-left">
          <div className="bg-slate-900 p-8 text-white flex justify-between items-center relative overflow-hidden">
            <div className="relative z-10 flex items-center gap-4">
              ...
              <div className="leading-none"><h4 className="font-black uppercase tracking-tight text-white mb-1">Consultant</h4><p className="text-[10px] text-orange-400 font-bold uppercase">Florida Office</p></div>
            </div>
      ...
      {!isAiChatOpen && (
        <button onClick={() => setIsAiChatOpen(true)} className="fixed bottom-6 right-6 sm:bottom-10 sm:right-10 bg-orange-500 text-white p-5 sm:p-6 rounded-[2rem] shadow-2xl hover:scale-110 active:scale-95 transition-all z-40">
