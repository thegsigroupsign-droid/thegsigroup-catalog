...
export default function App() {
  ...
  const [aiMessages, setAiMessages] = useState([{ 
    role: 'assistant', 
    text: `Hello! Welcome to ${COMPANY_NAME}. I'm your signage expert. How can I assist you today?` 
  }]);
  ...
  const handleSendMessage = async () => {
    ...
    const apiKey = ""; 
    const systemPrompt = `Expert consultant for ${COMPANY_NAME} in Florida. Categories: ${CATEGORIES.join(", ")}. Primary language is English. Speak English, Portuguese, or Spanish. Current items: ${products.map(p => p.name).join(", ")}`;
    ...
    try {
      ...
      const aiText = data?.candidates?.[0]?.content?.parts?.[0]?.text || "How can I help you today?";
      setAiMessages(prev => [...prev, { role: 'assistant', text: aiText }]);
    } catch (error) {
      setAiMessages(prev => [...prev, { role: 'assistant', text: "Service error. Please try again." }]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-orange-100">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:py-12">
          ...
          <div className="mb-12 space-y-8 animate-in slide-in-from-top-4 duration-500 text-left">
            <div className="bg-white border-2 border-orange-100 rounded-[2.5rem] p-6 sm:p-10 shadow-2xl">
              ...
              <form onSubmit={handleAddProduct} className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <button type="submit" className="md:col-span-3 bg-orange-600 text-white font-black py-5 rounded-[2rem] shadow-xl uppercase active:scale-95 transition-all text-sm tracking-widest">Publish to Website</button>
              </form>
            </div>
            <div className="bg-slate-900 rounded-[2.5rem] p-6 sm:p-10 text-white shadow-2xl overflow-hidden">
              <h3 className="text-xl font-bold mb-6 italic uppercase"><Layers className="w-5 h-5 inline mr-2 text-orange-500"/> Online Catalog ({products.length})</h3>
              <div className="max-h-[300px] overflow-y-auto no-scrollbar"><table className="w-full text-left"><tbody className="divide-y divide-white/5">{products.map(p => (<tr key={p.id} className="group hover:bg-white/5"><td className="py-4 px-2 font-bold text-sm">{p.name}</td><td className="py-4 px-2 text-[10px] text-slate-400 uppercase">{p.category}</td><td className="py-4 px-2 text-right"><button onClick={() => handleDeleteProduct(p.id)} className="text-slate-600 hover:text-red-500 p-2 transition-colors"><Trash2 className="w-5 h-5" /></button></td></tr>))}</tbody></table></div>
            </div>
          </div>
        )}
        ...
        {/* CATEGORY TABS: Flex-wrap and small fonts */}
        <div className="flex flex-wrap justify-center gap-2 mb-12 px-2">
          {CATEGORIES.map(cat => (
            <button
              ...
              } max-w-[130px] sm:max-w-none text-center leading-tight whitespace-normal min-h-[42px] flex items-center justify-center`}
            >
              {cat}
            </button>
          ))}
            ...
            <div key={product.id} className="group bg-white rounded-[2.5rem] sm:rounded-[3rem] border border-slate-100 p-4 hover:shadow-2xl transition-all duration-500 relative flex flex-col shadow-sm">
              ...
              <div className="flex-1 px-2 flex flex-col text-left">
                ...
                <div className="flex justify-between items-center mt-auto pt-5 border-t border-slate-50">
                  ...
                  <button onClick={() => setSelectedProduct(product)} className="bg-slate-900 text-white p-3.5 rounded-2xl hover:bg-orange-600 active:scale-90 transition-all shadow-lg"><Maximize className="w-4 h-4" /></button>
                </div>
              </div>
            </div>
          ))}
      ...
      {isPasswordModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/95 backdrop-blur-xl animate-in fade-in">
          <div className="bg-white rounded-[2.5rem] p-10 max-w-sm w-full shadow-2xl text-center space-y-6">
            ...
            <h2 className="text-2xl font-black italic uppercase text-slate-900">Admin Access</h2>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-900/80 backdrop-blur-md overflow-y-auto">
          <div className="bg-white rounded-[3rem] sm:rounded-[4rem] max-w-5xl w-full flex flex-col md:flex-row overflow-hidden shadow-2xl my-auto animate-in zoom-in duration-300">
            <div className="md:w-1/2 h-[350px] md:h-auto relative bg-slate-100"><img src={selectedProduct.image} className="w-full h-full object-cover" alt={selectedProduct.name} /><div className="absolute inset-0 bg-gradient-to-t from-black/30 to-transparent"></div></div>
        ...
        <div className="bg-white rounded-[2.5rem] sm:rounded-[3rem] shadow-2xl w-[320px] sm:w-[400px] h-[550px] sm:h-[650px] flex flex-col border border-slate-100 overflow-hidden text-slate-900">
          <div className="bg-slate-900 p-6 sm:p-8 text-white flex justify-between items-center relative overflow-hidden">
            <div className="relative z-10 flex items-center gap-3 sm:gap-4">
              ...
              <div className="text-left"><h4 className="font-bold text-sm tracking-wide text-white">AI Assistant</h4><p className="text-[9px] text-orange-400 font-bold uppercase tracking-widest">Florida Team</p></div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-6 bg-slate-50/50 no-scrollbar text-left">
            {aiMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] p-4 sm:p-5 rounded-[1.8rem] sm:rounded-[2.2rem] text-xs sm:text-sm leading-relaxed shadow-sm ${msg.role === 'user' ? 'bg-orange-600 text-white rounded-tr-none' : 'bg-white text-slate-700 rounded-tl-none border border-slate-100 text-slate-900'}`}>{msg.text}</div>
              </div>
            ))}
