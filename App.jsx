...
// Categorias oficiais para as abas e para o formulário
...
export default function App() {
  ...
  const handleSendMessage = async () => {
    ...
    const apiKey = ""; 
    const systemPrompt = `Você é um consultor especialista da ${COMPANY_NAME} na Flórida. Realizamos wraps, letreiros e design gráfico. Responda em PT, EN ou ES. Inventário atual: ${products.map(p => p.name).join(", ")}`;
    ...
    try {
      ...
      setAiMessages(prev => [...prev, { role: 'assistant', text: "Erro ao conectar com a IA." }]);
    } finally { 
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-orange-100">
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-slate-200 h-20 flex justify-between items-center px-4 sm:px-8 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="bg-slate-900 text-white px-6 py-2.5 rounded-2xl text-sm font-bold shadow-lg hover:bg-black transition-all">Contato</button>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 py-10">
          ...
          <div className="mb-16 space-y-8 animate-in slide-in-from-top-4 duration-500">
            <div className="bg-white border-2 border-orange-100 rounded-[3rem] p-8 shadow-2xl">
              <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => setIsAdminMode(false)} className="text-slate-400 hover:text-slate-900 font-bold text-xs uppercase tracking-widest bg-slate-100 px-5 py-2.5 rounded-xl transition-colors">Fechar</button>
              </div>
              ...
              <form onSubmit={handleAddProduct} className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <select className="p-4 bg-slate-50 border rounded-2xl outline-orange-500 appearance-none cursor-pointer" value={newProduct.category} onChange={e => setNewProduct({...newProduct, category: e.target.value})}>
                  {CATEGORIES.filter(c => c !== "Todos").map(c => <option key={c} value={c}>{c}</option>)}
                </select>
                <input placeholder="Preço Inicial ($)" type="number" className="p-4 bg-slate-50 border rounded-2xl outline-orange-500" value={newProduct.price} onChange={e => setNewProduct({...newProduct, price: e.target.value})} required />
                <input placeholder="Link da Imagem (PostImages.org)" className="md:col-span-3 p-4 bg-slate-50 border rounded-2xl outline-orange-500" value={newProduct.image} onChange={e => setNewProduct({...newProduct, image: e.target.value})} required />
                ...
                <button type="submit" className="md:col-span-3 bg-orange-600 text-white font-black py-5 rounded-[2rem] hover:bg-orange-700 transition-all shadow-xl shadow-orange-100 uppercase tracking-widest active:scale-95">Publicar Agora</button>
              </form>
            </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="text-[10px] uppercase text-slate-500 border-b border-white/5">
                      ...
                      <th className="pb-4 px-4 text-right">Apagar</th>
                    </tr>
                  </thead>
        ...
        {!isAdminMode && (
          <>
            <div className="mb-12 bg-[#F36F21] rounded-[3rem] p-10 text-white relative overflow-hidden flex flex-col md:flex-row items-center justify-between shadow-2xl shadow-orange-200/50">
              <div className="relative z-10 max-w-xl">
                <h2 className="text-4xl md:text-5xl font-black mb-4 leading-none uppercase tracking-tight text-white text-left">Destaque-se <br/><span className="text-neutral-900 underline decoration-4 underline-offset-8">na Flórida.</span></h2>
                <p className="text-orange-50 mb-8 text-lg font-medium opacity-90 leading-relaxed text-left">Sinalização de alto impacto e envelopamento profissional para o seu negócio.</p>
                <button onClick={() => setIsAiChatOpen(true)} className="bg-neutral-900 text-white px-8 py-4 rounded-2xl font-bold flex items-center gap-2 hover:bg-black transition-all shadow-xl"><MessageSquare className="w-5 h-5" /> Falar com Consultor IA</button>
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          ...
          {filteredProducts.map(product => (
            <div key={product.id} className="group bg-white rounded-[3.5rem] border border-slate-100 p-4 hover:shadow-2xl transition-all duration-500 flex flex-col relative">
              <div className="aspect-square rounded-[2.5rem] overflow-hidden bg-slate-50 mb-6 relative"><img src={product.image} alt={product.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000" /></div>
              <div className="flex-1 px-2 flex flex-col">
                <span className="text-[10px] font-bold text-orange-600 uppercase tracking-widest mb-1 text-left">{product.category}</span>
                <h3 className="font-bold text-slate-900 text-xl mb-3 group-hover:text-orange-600 transition-colors leading-tight text-left">{product.name}</h3>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/90 backdrop-blur-xl animate-in fade-in">
          <div className="bg-white rounded-[3rem] p-10 max-w-sm w-full shadow-2xl text-center space-y-6">
            ...
            <form onSubmit={handlePasswordSubmit} className="space-y-4">
              <input type="password" placeholder="SENHA" className={`w-full p-6 bg-slate-50 border-2 rounded-2xl text-center text-xl font-bold outline-none transition-all ${passwordError ? 'border-red-500 bg-red-50 animate-shake' : 'border-slate-100'}`} value={passwordInput} onChange={e => setPasswordInput(e.target.value)} autoFocus />
              <button type="submit" className="w-full p-5 bg-orange-600 text-white rounded-2xl font-black shadow-lg">Desbloquear</button>
              {passwordError && <p className="text-red-500 text-[10px] font-black uppercase tracking-widest animate-pulse mt-2 text-center">Chave Incorreta</p>}
            </form>
          </div>
        </div>
      )}
            ...
            <div className="md:w-1/2 p-10 md:p-16 flex flex-col justify-center relative">
              ...
              <div className="text-left">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-6 pt-10 border-t border-slate-100">
                  ...
                  <button onClick={() => window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=Quero um orçamento para ${selectedProduct.name}`)} className="bg-orange-600 text-white px-14 py-6 rounded-[2.5rem] font-black shadow-xl flex items-center gap-4 active:scale-95 transition-all">Orçamento <ExternalLink className="w-5 h-5" /></button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-[3.5rem] shadow-2xl w-[360px] sm:w-[420px] h-[650px] flex flex-col border border-slate-100 overflow-hidden text-slate-900">
          <div className="bg-slate-900 p-8 text-white flex justify-between items-center relative overflow-hidden">
            <div className="relative z-10 flex items-center gap-4">
              ...
              <div className="text-left"><h4 className="font-bold text-sm tracking-wide text-white">Assistente IA</h4><p className="text-[10px] text-orange-400 font-bold uppercase tracking-widest text-left">Equipa Flórida</p></div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-slate-50/50 no-scrollbar">
            {aiMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[85%] p-5 rounded-[2rem] text-sm leading-relaxed shadow-sm text-left ${msg.role === 'user' ? 'bg-orange-600 text-white rounded-tr-none' : 'bg-white text-slate-700 rounded-tl-none border border-slate-100'}`}>{msg.text}</div></div>
            ))}
          </div>
