        ...
        <div className="flex flex-col gap-4 sm:gap-6 mb-8 sm:mb-12">
          ...
          <div className="sm:hidden relative">
            <button onClick={() => setIsMobileCategoryMenuOpen(!isMobileCategoryMenuOpen)} className="w-full bg-white border border-slate-200 p-4 rounded-xl flex items-center justify-between font-black text-[11px] uppercase tracking-widest shadow-sm">
              <div className="flex items-center gap-2"><Filter className="w-4 h-4 text-orange-500" /> Category: <span className="text-orange-600">{filter}</span></div>
            ...
            {isMobileCategoryMenuOpen && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-xl shadow-2xl z-50 py-2">
                {CATEGORIES.map(cat => (
                  <button key={cat} onClick={() => { setFilter(cat); setIsMobileCategoryMenuOpen(false); }} className={`w-full text-left px-5 py-3.5 text-[10px] font-black uppercase tracking-widest ${filter === cat ? 'bg-orange-50 text-orange-600' : 'text-slate-500'}`}>{cat}</button>
                ))}
              </div>
            )}
          </div>
            ...
            <div className="md:w-1/2 p-6 sm:p-16 flex flex-col justify-center relative bg-white flex-1">
              ...
              <div>
                ...
                <div className="bg-slate-50 p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] mb-8 border border-slate-100 shadow-inner">
                  <p className="text-slate-600 leading-relaxed text-sm sm:text-lg italic">"{selectedProduct.description}"</p>
                </div>
